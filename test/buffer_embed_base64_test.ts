...
describe("Embedded Buffer", function () {
  ...
  it(" is defined", function () {
    expect( gltf.getElement( GltfTypes.BUFFER, 0 )).to.be.ok()
  });


  it(" have correct length", function () {
    expect( gltf.getElement( GltfTypes.BUFFER, 0 ).byteLength ).to.be(60)
    expect( gltf.getElement( GltfTypes.BUFFER, 0 )._bytes.byteLength ).to.be(60)
  })

  it(" have correct data", function () {
    const u32 = new Uint32Array( gltf.getElement( GltfTypes.BUFFER, 0 )._bytes );
